...
function displayDetails(submission) {
  ...
  const details = [
    ...
    { label: "Resume", value: submission.resumeFilename && submission.resumeFilename.trim().length > 0 ? `<a class="resume-link" href="preview.html?file=${encodeURIComponent(submission.resumeFilename)}">${submission.resumeFilename}</a>` : "Not uploaded" },
